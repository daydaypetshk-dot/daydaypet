...
export async function POST(req: NextRequest) {
  ...
  let body: { id?: string } & Partial<PetInsert>;
  try {
    body = (await req.json()) as { id?: string } & Partial<PetInsert>;
  } catch (err) {
    console.error("POST /api/admin/update-pet invalid JSON:", err);
  ...
  const updatePayload: Partial<PetInsert> = { ...body };
  ...
  const { data: existing, error: existingError } = await withTimeout(
    ...
    "[admin/update-pet] lookup",
  );
  if (existingError) {
    console.error("[admin/update-pet] lookup error:", existingError);
  ...
  console.log("[admin/update-pet] update payload:", {
    petId,
    updatePayload,
  ...
  try {
    const result = await withTimeout(
      admin.from("pets").update(updatePayload).eq("id", petId).select("*").single(),
      SUPABASE_QUERY_TIMEOUT_MS,
      "[admin/update-pet] update",
    );
    if (result.error) throw result.error;
    updated = result.data;
  } catch (err: any) {
    console.error("[admin/update-pet] database update error:", err);
  ...
  if (approvalTransition) {
    ...
    try {
      ...
      console.error("[admin/update-pet] post-approval notify error:", notifyErr);
      try {
        await createNotificationDispatchLog({
          petId: updated.id,
          ownerUserId: updated.user_id ?? null,
