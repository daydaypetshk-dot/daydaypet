...
export async function GET() {
  try {
    ...
    if (!guard.ok) {
      ...
      return NextResponse.json({ ok: false, reason, error: guard.error, cookieNames }, { status: guard.status });
    }
    return NextResponse.json({ ok: true, userId: guard.user.id, email: guard.user.email, role: guard.role, cookieNames });
  } catch (error) {
